...
def _check_body_mods(bm: Mapping):
    ...
    if REQ_BODY_KEY not in bm or not _is_integer(REQ_BODY_KEY) or bm[REQ_BODY_KEY] <= 0:
        raise ValueError(f"BODY_MODS.BASE_SIZE_CM3 must be positive int")
    for k in (OPT_BMOD_KEYS & set(bm.keys())):
        v = bm[k]
        if (not isinstance(v, (list, tuple)) or len(v) != 2 or not all(_is_integer(x) for x in v) or v[0] <= 0 or v[0] > v[1]):
